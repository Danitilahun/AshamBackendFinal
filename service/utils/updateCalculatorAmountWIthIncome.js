 ...
 */
const updateCalculatorAmountWithIncome = async (db, batch, id, newIncome) => {
  try {
    if (!id) {
      throw new Error(
    ...
    console.log(data);
    const incomeDifference = newIncome - data.income;
    const newAmount = data.actual + newIncome;
    const newBalance = data.sum - newAmount;
...
module.exports = updateCalculatorAmountWithIncome;
